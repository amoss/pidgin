...
def build():
    ...
    g.addRule("bounded2",  [g.Nonterminal("bounded3",strength="all", modifier="some"), g.TermString("b")])
    g.addRule("bounded3",  [g.TermSet(set(['x','y']))])
    g.addRule("unbounded",   [g.Nonterminal("unbounded2",strength="frugal", modifier="any")])
    g.addRule("unbounded2",  [g.Nonterminal("unbounded3",strength="all", modifier="some")])
