...
class Grammar:
    ...
    def build(self):
        ...
        for config in self.worklist:
            ...
            if config.position < len(config.clause.rhs):
                ...
                if isinstance(sym, self.Terminal) or isinstance(sym, self.CalcTerminal):
    ...
    class CalcTerminal:
        ...
        def __eq__(self, other):
            return isinstance(other,Grammar.CalcTerminal) and self.name==other.name and self.modifier==other.modifier
